            ...
            <div v-for="task in tasks" :key="task.id">
                <div class="bg-white shadow-lg rounded-lg p-6">
                    <div class="flex flex-row justify-between">
                        <h2 class="text-xl font-semibold mb-4" v-if="!task.editing" @click="editTask(task)">{{
                    task.title }}</h2>
                        <input v-model="updatedTask" v-else @keyup.enter="updateTask(task)" @blur="cancelEditing(task)"
                            class="text-xl font-semibold mb-4 p-2" :class="task.editing ? ' ring-1  black' : ''" />
                        <button @click="editTask(task)" class="text-green-500 hover:text-green-700">Edit</button>
                    </div>
